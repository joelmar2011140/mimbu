...
export function verificarDatas (dataInicio: string, dataFim: string): void {
  ...
  if (!dataInicioNormalizada.isValid() || !dataFimNormalizada.isValid()) {
    throw new ApiError('CLIENT_ERROR', 'Indique por favor uma data válida, a data deve estar neste formato (2023-04-12)', 400)
  }
  // A Data de ínicio não pode estar antes do dia atual
  if (dataInicioNormalizada.isBefore(hoje)) {
    throw new ApiError('CLIENT_ERROR', 'Indique por favor uma data de ínicio válida, pois a data indicada parece já ter passado', 400)
  }
  // A Data de fim  não pode estar antes do dia atual
  if (dataFimNormalizada.isBefore(hoje)) {
    throw new ApiError('CLIENT_ERROR', 'Indique por favor uma data de fim válida,pois a data indicada parece já ter passado', 422)
  }
  // A data de início não pode estar depois da data de fim
  if (dataInicioNormalizada.isAfter(dataFimNormalizada)) {
    throw new ApiError('CLIENT_ERROR', 'A Data de ínicio não pode estar depois da data de fim de evento', 422)
  }
  // A data de fim não pode estar antes da data de início
  if (dataFimNormalizada.isBefore(dataInicioNormalizada)) {
    throw new ApiError('CLIENT_ERROR', 'A Data de fim não pode estar antes da data de início de evento', 422)
  }
}
  ...
  switch (tipo) {
    case 'dataInicio':
      if (!dataNormalizada.isValid()) {
        throw new ApiError('CLIENT_ERROR', 'Indique por favor uma data de início válida, a data deve estar neste formato (2023-04-12)', 422)
      }
      if (dataNormalizada.isBefore(hoje)) {
        throw new ApiError('CLIENT_ERROR', 'Indique por favor uma data de ínicio válida, pois a data indicada parece já ter passado', 422)
      }
    ...
    case 'datafim':
      if (!dataNormalizada.isValid()) {
        throw new ApiError('CLIENT_ERROR', 'Indique por favor uma data de fim válida, a data deve estar neste formato (2023-04-12)', 422)
      }
      if (dataNormalizada.isBefore(hoje)) {
        throw new ApiError('CLIENT_ERROR', 'Indique por favor uma data de fim válida, pois a data indicada parece já ter passado', 422)
      }
