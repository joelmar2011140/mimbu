...
import { fetchEdicoesParam, fetchVotantesParam } from '@/lib/fetch.functions';
...
export const TableVotantes = () => {
  ...
  const [isOpenDelete, setIsOpenDelete] = useState(false)
  const { data, isLoading } = useQuery({
    queryKey: ['fetchVotantes', pagina, porPagina],
    queryFn: async () => await fetchVotantesParam(pagina, porPagina),
    keepPreviousData: true
  })
